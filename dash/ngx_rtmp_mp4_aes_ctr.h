...
ngx_int_t mp4_aes_ctr_process( mp4_aes_ctr_state_t* state, u_char* dest, const u_char* src, uint32_t size);
void mp4_aes_ctr_increment_be64(u_char* counter);
ngx_int_t mp4_aes_ctr_write_encrypted(mp4_aes_ctr_state_t* state, write_buffer_state_t* write_buffer, u_char* cur_pos, uint32_t write_size);
